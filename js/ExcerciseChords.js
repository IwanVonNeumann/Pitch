var ExcerciseChords = 
{
  ...
  resetButtonLabels: function()
  {
    ...
    var num_btns = this.isLevelMajor() ? SequenceChordTypes.num_maj_chords : SequenceChordTypes.num_min_chords;
    ...
    for (var i = 0; i < num_btns; ++i)
    {
      ...
      var chn = i;
      if (this.isLevelMajor())
        chn += SequenceChordTypes.minmaj_shift;
      var ch = SequenceChordTypes.createChord(12, chn, this.root);
      ...
      for (var ind = 0; ind < (this.getCorrectedLevel()>>1) + 3; ++ind)
        if (this.isLevelMajor())
        {
          if (SequenceChordTypes.major_chords[ind] == i + SequenceChordTypes.minmaj_shift)
            this.chord_btns[i].disabled = false;
        }
        else
          if (SequenceChordTypes.minor_chords[ind] == i)
            this.chord_btns[i].disabled = false;

      var text = SequenceChordTypes.chordName(chn) + "<br>" + Constants.toneNameInKey(ch.root, this.root, this.getCorrectedLevel() % 2 != 0);

      if (ch.type == ChordTypes.min)
        text += "m";

      if (ch.type == ChordTypes.dom7)
        text += "7";

      this.chord_btns[i].innerHTML = text;
      this.chord_btns[i].onmousedown  = (function(ii) {return function (){console.log("mouse down" + ii.toString()); ExcerciseChords.buttonDown(ii);}}) (i);
  ...
  {
    return SequenceChordTypes.num_min_chords + SequenceChordTypes.num_maj_chords - 4;
  },
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
  ...
  {
    var ch = SequenceChordTypes.createChord(12, chord, this.root);
    var text = SequenceChordTypes.chordName(chord);
  ...
  getPromptText: function()
  {
    var text;
    text = "";
    text += SequenceChordTypes.chordName(this.current_seq.sequence[0]);
